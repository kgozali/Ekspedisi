...
Public Class booking_do
    ...
    Private Sub booking_do_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            ...
            data = DtTable("Select id_booking `Kode Booking`,concat(day(tgl),'-',monthname(tgl),'-',year(tgl)) `Tanggal Pengiriman`,jam `Jam Pengiriman`,concat(ETA,' ','Jam') `ETA`,nama_principle `Principle`,concat(kota_asal,' - ',kota_tujuan) `Rute`,keterangan `Keterangan` from booking_truk,mprinciple,mrute where booking_truk.id_principle=mprinciple.id_principle and booking_truk.id_rute=mrute.id_rute and booking_truk.s=1 and booking_truk.del=0 order by tgl asc,jam asc")
    ...
    Sub selection()
        ...
        Try
            If id.Checked = True Then
                ...
                dataset = DtTable("Select id_booking `Kode Booking`,concat(day(tgl),'-',monthname(tgl),'-',year(tgl)) `Tanggal Pengiriman`,jam `Jam Pengiriman`,concat(ETA,' ','Jam') `ETA`,nama_principle `Principle`,concat(kota_asal,' - ',kota_tujuan) `Rute`,keterangan `Keterangan` from booking_truk,mprinciple,mrute where booking_truk.id_principle=mprinciple.id_principle and booking_truk.id_rute=mrute.id_rute and id_booking LIKE '%" + cari.Text + "%' and booking_truk.s=1 and booking_truk.del=0")
            ...
            ElseIf principle.Checked = True Then
                ...
                cari.Visible = True
                Dim dataset As New DataTable
                dataset = DtTable("Select id_booking `Kode Booking`,concat(day(tgl),'-',monthname(tgl),'-',year(tgl)) `Tanggal Pengiriman`,jam `Jam Pengiriman`,concat(ETA,' ','Jam') `ETA`,nama_principle `Principle`,concat(kota_asal,' - ',kota_tujuan) `Rute`,keterangan `Keterangan` from booking_truk,mprinciple,mrute where booking_truk.id_principle=mprinciple.id_principle and booking_truk.id_rute=mrute.id_rute and nama_principle LIKE '%" + cari.Text + "%' and booking_truk.s=1 and booking_truk.del=0")
                controlbooking.DataSource = dataset
