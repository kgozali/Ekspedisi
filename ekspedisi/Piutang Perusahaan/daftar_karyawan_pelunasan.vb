Public Class daftar_karyawan_pelunasan 
    ...
    Private Sub Submit_Click(sender As Object, e As EventArgs) Handles Submit.Click
        Try
            ...
            tabel = DtTablebayar("SELECT p.id_piutangkaryawan as `Kode Piutang`,tgl `Tanggal Piutang`,jatuh_tempo `Tanggal Jatuh Tempo`,nominal `Nominal`,keterangan `Keterangan`,if(sum(jumlah_dibayar) is null,0,sum(jumlah_dibayar)) as `Terbayar`,if(nominal-sum(jumlah_dibayar) is null or nominal-sum(jumlah_dibayar)=nominal,0,nominal-sum(jumlah_dibayar)) as `Sisa` FROM dpiutang_karyawan d right join piutang_karyawan p on d.id_piutangkaryawan=p.id_piutangkaryawan and status='1' and p.id_karyawan='" & pelunansan_piutang_karyawan.namakaryawan.Text & "' group by p.id_piutangkaryawan;")
