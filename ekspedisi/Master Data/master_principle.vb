...
Public Class master_principle
    ...
    Private Sub master_principle_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        For i = 0 To GridView1.DataRowCount - 1
            Dim temp As String = GridView1.GetRowCellValue(i, "ID principle").ToString
            ...
            If checks.Rows.Count > 0 Then
                For i = 0 To GridView2.DataRowCount - 1
                    For j = 0 To checks.Rows.Count() - 1
                        If GridView2.GetRowCellValue(i, "ID principle").ToString = checks.Rows(j).Item(0).ToString Then
            ...
            If cek = True Then
                Select Case MsgBox("Apakah anda yakin menghapus data ini?", MsgBoxStyle.YesNo, "System Error")
                    Case MsgBoxResult.Yes
                        For i = 0 To GridView2.RowCount - 1
                            If GridView2.IsRowSelected(i) = True Then
                                InsertInto("update mprinciple set `s`='0' where id_principle='" & GridView2.GetRowCellValue(GridView1.FocusedRowHandle, "ID principle").ToString & "'")
                            End If
                        Next i
        ...
        Try
            If GridView2.IsRowSelected(GridView2.FocusedRowHandle) Then
                For i = 0 To unchecks.Rows.Count() - 1
                    If unchecks.Rows(i).Item(0).ToString = GridView2.GetRowCellValue(GridView2.FocusedRowHandle, "ID principle").ToString Then
            ...
            Else
                For i = 0 To checks.Rows.Count() - 1
                    If checks.Rows(i).Item(0).ToString = GridView2.GetRowCellValue(GridView2.FocusedRowHandle, "ID principle").ToString Then
