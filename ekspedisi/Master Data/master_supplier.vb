...
Public Class master_supplier
    ...
    Private Sub master_supplier_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        GridControl2.Visible = False
        data = DtTable("SELECT s.id_supplier `ID supplier`, s.nama_supplier `Nama supplier`, s.Alamat `Alamat`, s.Email, s.tel1`Telepon 1`, s.tel2 `Telepon 2`, s.Kota, s.Provinsi, m.nama_kategori `Nama Kategori` from msupplier s, mkategori m where s.`s`='1'")
        GridControl1.DataSource = data
        For i = 0 To data.Columns.Count - 1
            GridView1.Columns(i).OptionsColumn.AllowEdit = False
        Next

        checks.Columns.Add("ID supplier")
        unchecks.Columns.Add("ID supplier")
        For i = 0 To GridView1.DataRowCount - 1
            Dim temp As String = GridView1.GetRowCellValue(i, "ID supplier").ToString
        ...
        If edit.Down = True Then
            If id.Checked = True Then
                If aktif.Checked = True Then
                    data = DtTable("SELECT s.id_supplier `ID supplier`, s.nama_supplier `Nama supplier`, s.Alamat `Alamat`, s.Email, s.tel1`Telepon 1`, s.tel2 `Telepon 2`, s.Kota, s.Provinsi, m.nama_kategori `Nama Kategori` from msupplier s, mkategori m where s.`s`='1' and s.id_supplier like '%" & cari.Text & "%'")
                Else
                    data = DtTable("SELECT s.id_supplier `ID supplier`, s.nama_supplier `Nama supplier`, s.Alamat `Alamat`, s.Email, s.tel1`Telepon 1`, s.tel2 `Telepon 2`, s.Kota, s.Provinsi, m.nama_kategori `Nama Kategori` from msupplier s, mkategori m where s.`s`='0' and s.id_supplier like '%" & cari.Text & "%'")
                End If

            Else
                If aktif.Checked = True Then
                    data = DtTable("SELECT s.id_supplier `ID supplier`, s.nama_supplier `Nama supplier`, s.Alamat `Alamat`, s.Email, s.tel1`Telepon 1`, s.tel2 `Telepon 2`, s.Kota, s.Provinsi, m.nama_kategori `Nama Kategori` from msupplier s, mkategori m where s.`s`='1' and s.nama_supplier like '%" & cari.Text & "%'")
                Else
                    data = DtTable("SELECT s.id_supplier `ID supplier`, s.nama_supplier `Nama supplier`, s.Alamat `Alamat`, s.Email, s.tel1`Telepon 1`, s.tel2 `Telepon 2`, s.Kota, s.Provinsi, m.nama_kategori `Nama Kategori` from msupplier s, mkategori m where s.`s`='0' and s.nama_supplier like '%" & cari.Text & "%'")
                End If
            End If
            GridControl2.DataSource = data
            ...
            If checks.Rows.Count > 0 Then
                For i = 0 To GridView2.DataRowCount - 1
                    For j = 0 To checks.Rows.Count() - 1
                        If GridView2.GetRowCellValue(i, "ID supplier").ToString = checks.Rows(j).Item(0).ToString Then
            ...
            If cek = True Then
                Select Case MsgBox("Apakah anda yakin menghapus data ini?", MsgBoxStyle.YesNo, "System Error")
                    Case MsgBoxResult.Yes
                        For i = 0 To GridView2.RowCount - 1
                            If GridView2.IsRowSelected(i) = True Then
                                InsertInto("update msupplier set `s`='0' where id_supplier='" & GridView2.GetRowCellValue(GridView1.FocusedRowHandle, "ID supplier").ToString & "'")
                            End If
                        Next i
        ...
        Try
            If GridView2.IsRowSelected(GridView2.FocusedRowHandle) Then
                For i = 0 To unchecks.Rows.Count() - 1
                    If unchecks.Rows(i).Item(0).ToString = GridView2.GetRowCellValue(GridView2.FocusedRowHandle, "ID supplier").ToString Then
            ...
            Else
                For i = 0 To checks.Rows.Count() - 1
                    If checks.Rows(i).Item(0).ToString = GridView2.GetRowCellValue(GridView2.FocusedRowHandle, "ID supplier").ToString Then
