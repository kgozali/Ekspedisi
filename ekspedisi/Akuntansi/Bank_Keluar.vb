...
Public Class Bank_Keluar
    ...
    Public Function initform(ByVal modelayar As Integer, ByVal KeyID As String) As Boolean
        ...
        Select Case modelayar
            Case 1
                Me.Text = "Input Bank Keluar"
            Case 2
                loaddata(_mkeyid)
                Me.Text = "Edit Bank Keluar"
            Case 3
                loaddata(_mkeyid)
                Me.Text = "Hapus Bank Keluar"
        End Select
    ...
    Private Sub Bank_Keluar_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub Submit_Click(sender As Object, e As EventArgs) Handles Submit.Click
        ...
        Try
            ...
            If flagval Then
                ...
                command.ExecuteNonQuery()
                command.CommandText = "insert into trans_bank (id_tbank,tgl,id_akun_bank,keterangan,cetakan_ke,tipe_dokumen) VALUES ('" & TextBox1.Text & "','" & Format(DateTimePicker1.Value, "yyyy-MM-dd") & "','" & GridLookUpEdit1.EditValue & "','" & CekTandaPetik(RichTextBox1.Text) & "',0,'K')"
                command.ExecuteNonQuery()
                'insert detil
            ...
            If flagval And savestatus = True Then
                ...
                If _modelayar = 1 Then
                    ...
                    If IsNothing(daftar_bank_keluar) = False Then
                        daftar_bank_keluar.refreshgrid("")
                    End If
                Else
                    Me.Close()
                End If
    ...
    Private Function gennobuktina(ByVal tgl As Date) As String
        ...
        retval = autogenerate2("BK", "select max(id_tbank) from trans_bank where tipe_dokumen='K' and month(tgl)=" & Format(tgl, "MM") & " and year(tgl)=" & Format(tgl, "yyyy"), tgl)
        Return retval
    End Function
    ...
    Private Sub Bank_Keluar_Shown(sender As Object, e As EventArgs) Handles Me.Shown
        If _modelayar = 3 Then
            Dim result As DialogResult = MessageBox.Show("Hapus Data ini ?", _
                              "Transaksi Bank Keluar", _
