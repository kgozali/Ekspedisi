...
Public Class kas_keluar
    ...
    Public Function initform(ByVal modelayar As Integer, ByVal KeyID As String) As Boolean
        ...
        Select Case modelayar
            Case 1
                Me.Text = "Input Kas Keluar"
            Case 2
                loaddata(_mkeyid)
                Me.Text = "Edit Kas Keluar"
            Case 3
                loaddata(_mkeyid)
                Me.Text = "Hapus Kas Keluar"
        End Select
    ...
    Private Sub kas_keluar_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub Submit_Click(sender As Object, e As EventArgs) Handles Submit.Click
        ...
        Try
            ...
            If flagval Then
                ...
                command.ExecuteNonQuery()
                command.CommandText = "insert into trans_kas (id_transaksi_kas,tgl,id_akun_kas,keterangan,cetakan_ke,tipe_dokumen) VALUES ('" & TextBox1.Text & "','" & Format(DateTimePicker1.Value, "yyyy-MM-dd") & "','" & GridLookUpEdit1.EditValue & "','" & CekTandaPetik(RichTextBox1.Text) & "',0,'K')"
                command.ExecuteNonQuery()
                'insert detil
                For i = 0 To DataSet1.Tables.Item("datadetil").Rows.Count - 1
                    ...
                    command.ExecuteNonQuery()
                    command.CommandText = "insert into djurnal (no_jurnal,id_akun,keterangan,nominal) VALUES ('" & TextBox1.Text & "','" & dtrow("id_akun_detil") & "','" & CekTandaPetik(dtrow("keterangan")) & "'," & dtrow("nominal") & ")"
                    command.ExecuteNonQuery()
                Next
                Dim total As Double = GridView1.Columns("nominal").SummaryItem.SummaryValue
                command.CommandText = "insert into djurnal (no_jurnal,id_akun,keterangan,nominal) VALUES ('" & TextBox1.Text & "','" & GridLookUpEdit1.EditValue & "','" & CekTandaPetik(RichTextBox1.Text) & "'," & -1 * total & ")"
                command.ExecuteNonQuery()
        ...
        Finally
            ...
            If flagval And savestatus = True Then
                ...
                MessageBox.Show("Simpan Data Kas Keluar " + TextBox1.Text + " Sukses", "System Notification", MessageBoxButtons.OK, MessageBoxIcon.Information)
                If _modelayar = 1 Then
                    ResetValues()
                    If IsNothing(daftar_kas_keluar) = False Then
                        daftar_kas_keluar.refreshgrid("")
                    End If
                Else
                    Me.Close()
                End If
    ...
    Private Function gennobuktina(ByVal tgl As Date) As String
        ...
        retval = autogenerate2("KK", "select max(id_transaksi_kas) from trans_kas where tipe_dokumen='K' and month(tgl)=" & Format(tgl, "MM") & " and year(tgl)=" & Format(tgl, "yyyy"), tgl)
        Return retval
    End Function
    ...
    Private Sub kas_keluar_Shown(sender As Object, e As EventArgs) Handles Me.Shown
        If _modelayar = 3 Then
            Dim result As DialogResult = MessageBox.Show("Hapus Data ini ?", _
                              "Transaksi Kas Keluar", _
